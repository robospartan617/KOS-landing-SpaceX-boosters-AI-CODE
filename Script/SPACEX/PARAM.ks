...
if not pLex:haskey("payloadType") { pLex:add("payloadType",1). }
if not pLex:haskey("landProfile") { pLex:add("landProfile",0). }
...
// ── SELF-CALIBRATING LZ CORRECTION ───────────────────────────────────────
// After each landing, BOOSTER.ks writes the landing error to lz_cal.json.
// Here we apply the accumulated correction to all three LZ targets,
// shifting them by the measured drift so each run gets closer to the pad.
// On the first run (no file yet) correction is zero — no effect.
// ── SELF-CALIBRATING LZ CORRECTION ──────────────────────────────────────
// BOOSTER.ks writes lz_cal.json after each landing with the average error
// between where it aimed and where it actually landed.
// We apply that correction to the BASE KSC coordinates (not the already-
// corrected value from last run) so errors don't compound across launches.
local KSC_BASE_LAT is -0.0972.
local KSC_BASE_LNG is -74.5757.
local dLat is 0.
local dLng is 0.
if exists("0:/lz_cal.json") {
    local cal is readjson("0:/lz_cal.json").
    set dLat to cal["errLat"].
    set dLng to cal["errLng"].
    print "LZ cal from " + cal["runs"] + " run(s): dLat=" + round(dLat,4) + " dLng=" + round(dLng,4).
}
// Always rebuild LZ from raw base + current correction delta
local correctedLZ is latlng(KSC_BASE_LAT + dLat, KSC_BASE_LNG + dLng).
set pLex["LZ0"] to correctedLZ.
set pLex["LZ1"] to correctedLZ.
set pLex["LZ2"] to correctedLZ.
