...
if exists("0:/lz_cal.json") {
    ...
    local dLat is cal["errLat"].
    local dLng is cal["errLng"].
    if (abs(dLat) > 0 or abs(dLng) > 0) {
        print "Applying LZ correction from " + cal["runs"] + " previous run(s).".
        print "  dLat=" + round(dLat,4) + "  dLng=" + round(dLng,4).
        set pLex["LZ0"] to latlng(pLex["LZ0"]:lat + dLat, pLex["LZ0"]:lng + dLng).
        set pLex["LZ1"] to latlng(pLex["LZ1"]:lat + dLat, pLex["LZ1"]:lng + dLng).
        set pLex["LZ2"] to latlng(pLex["LZ2"]:lat + dLat, pLex["LZ2"]:lng + dLng).
    }
}
