...
function Boostback {
    ...
    if (landProfile = 1 or core:tag = "2" or core:tag = "3") {
        ...
        local landingOvershoot is -2000 - ((maxPayload - payloadMass) / 7).	// aim 2km past LZ to compensate reentry drift
        ...
        local tempLZ is LZ.
        local landingOvershoot is 1500.	// aim 1.5km past LZ for ASDS profile
...
function Land {
    ...
    // Engines-down: -up:vector points nose toward ground
    ...
    until (alt:radar < 5 or ship:verticalspeed > -1) { wait 0. }
    ...
    // Blend new error with history (weighted average, more weight on recent)
    local runs is calLex["runs"] + 1.
    local blendWeight is min(0.6, 1 / runs).   // reduces over time as we converge
    local newErrLat is (calLex["errLat"] * (1 - blendWeight)) + (errLat * blendWeight).
    local newErrLng is (calLex["errLng"] * (1 - blendWeight)) + (errLng * blendWeight).
