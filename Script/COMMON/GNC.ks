...
function LandThrottle {		// throttle for landing
	...
	// Net upward acceleration available (thrust minus gravity)
	local shipAcc is (ship:availableThrust / ship:mass) - (body:mu / body:position:sqrmagnitude).
	// Guard: if thrust can't overcome gravity, clamp to small positive so we don't divide by zero
	set shipAcc to max(shipAcc, 0.1).

	// Use ONLY vertical speed — horizontal drift is corrected by steering, not throttle.
	// Using total speed caused the booster to hover and spin at low altitude.
	local vertSpd is abs(ship:verticalspeed).
	// 1.5x safety margin accounts for engine spool-up delay and IntegLand timing jitter.
	// Without margin the fast booster consistently starts the burn too late and crashes.
	local stopDist is (vertSpd^2 / (2 * shipAcc)) * 1.5.

	return (stopDist / max(ship:bounds:bottomaltradar, 1)).
}
...
function IntegLand {		// landing height check through integration (fixed drag/mass)
	...
	// Scale drag area by mass (~0.08 m²/t is reasonable for a Falcon 9-sized booster).
	// Increase this value if your booster is wider; decrease if it is slender.
	local shipACD       is ship:mass * 0.08.
	...
	if (simHeight <= 0 or stepCount > maxSteps) { return true. }	// start burn NOW
	if (simHeight > 50) { return false. }	// still too high — wait longer
	else { return true. }
}
